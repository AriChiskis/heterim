...
def to_scalar(arr):
    """
    Converts a one-element NumPy array to a scalar.

    Args:
        arr (numpy.ndarray): A NumPy array containing exactly one element.

    Returns:
        scalar: The scalar value of the single element in the array.

    Raises:
        ValueError: If the input array contains more than one element.
    """
    if arr.size != 1:
        raise ValueError("The input array must contain exactly one element.")
    return arr.item()
